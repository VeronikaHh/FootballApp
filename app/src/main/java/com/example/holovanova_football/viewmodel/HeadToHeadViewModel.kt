...
class HeadToHeadViewModel @Inject constructor(
    ...
    fun collectFlow(firstTeamId: Int, secondTeamId: Int) {
        ...
        viewModelScope.launch(Dispatchers.Default) {
            val lastTen = headToHeadRepository.getHeadToHead(h2h)
            _lastTen.emit(lastTen)
            val thisSeason = headToHeadRepository.getHeadToHeadThisSeason(h2h)
            _thisSeason.emit(thisSeason)
            val lastSeason = headToHeadRepository.getHeadToHeadLastSeason(h2h)
            _lastSeason.emit(lastSeason)
            isDataFetched = true
            combine(
                _lastSeason,
                _thisSeason,
